...
public class CertainBookStore implements BookStore, StockManager {
	...
	public Map<Integer, BookStoreBook> bookMap = null;
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		//check that no of top books requested is positive
		if (numBooks < 0) {
			throw new BookStoreException("numBooks = " + numBooks + ", but it must be positive");
		}
		...
		Iterator<Entry<Integer, BookStoreBook>> it = bookMap.entrySet().iterator();
		// Get all books
		List<BookStoreBook> allBooks = new ArrayList<BookStoreBook>();
		while (it.hasNext()) {
			Entry<Integer, BookStoreBook> pair = it.next();
			BookStoreBook book = pair.getValue();
			allBooks.add(book);
		}
		// Sort all books by the average rating
        Collections.sort(allBooks, new Comparator<BookStoreBook>() {
		    public int compare(BookStoreBook one, BookStoreBook other) {
		        return Float.compare(one.getAverageRating(),other.getAverageRating());
		    }
		});
        // Reverse the order of sorted books in descending order
        Collections.reverse(allBooks);
        List<BookStoreBook> topBooks = allBooks.subList(0, numBooks);
        
		List<Book> listTopBooks = new ArrayList<>();
		for (BookStoreBook topBook : topBooks) {
			listTopBooks.add(bookMap.get(topBook.getISBN()).immutableBook());
		}
	...
	public synchronized void rateBooks(Set<BookRating> bookRating) throws BookStoreException {
		...
		//check that all information is correct before rating books in order to respect all-or-nothing semantics
		for(BookRating selectedBookRating : bookRating) {
			int isbn = selectedBookRating.getISBN();
			int rating = selectedBookRating.getRating();
		...
		for(BookRating selectedBookRating : bookRating){
			BookStoreBook book = bookMap.get(selectedBookRating.getISBN());
			book.addRating(selectedBookRating.getRating());
		}
	}
